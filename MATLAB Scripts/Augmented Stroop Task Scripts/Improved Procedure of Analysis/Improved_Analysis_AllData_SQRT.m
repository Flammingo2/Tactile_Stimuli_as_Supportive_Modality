...
data1 = read(data1);
data2 = read(data2);
% Combine Boße's and our Dataset
data = [data1; data2];
...
number_artefacts = height(toDelete)
data(toDelete,:) = [];

% FINAL For plotting accuracy
writetable(data, 'replication_MyAnalysis_all_data_to_plot_accuracy.csv');
...
%% Transformation on Accuracy and Reaction Time
...
% Accuracy reflect and LOG-transformation
% left-skewed distribution
max = max(means_data.accuracy_mean);
data_accuracy_reflected = array2table((max - means_data.accuracy_mean), ...
                    'VariableNames', {'reflected_accuracy_mean'});
data_accuracy_reflected_sqrt = array2table(sqrt(data_accuracy_reflected.reflected_accuracy_mean), ...
                    'VariableNames', {'sqrt_reflected_accuracy_mean'});
means_data_sqrt = [means_data data_accuracy_reflected_sqrt];
...
for pointer = 1:26  
    ...
    vp = means_data_sqrt(means_data_sqrt.VP_Nr == subject, :);
    % Store the eight conditions horizontally and concatenate all of them vertically
    data_accuracy = [data_accuracy;[vp.sqrt_reflected_accuracy_mean']];
    
end

% Convert to a table
data_accuracy = array2table(data_accuracy, 'VariableNames', {'day1_congruent_tactile0','day1_congruent_tactile1', 'day1_incongruent_tactile0','day1_incongruent_tactile1','day2_congruent_tactile0','day2_congruent_tactile1','day2_incongruent_tactile0','day2_incongruent_tactile1'});
data_accuracy
        ...
        for pointer2 = 1:2
            ...
            for modality = 0:1
                ...
                conditions_RT.(strcat('subject_',num2str(subject), '_',...
                                num2str(column))) = condition.log_reaction_time;
                column = column + 1;
            end
        end
    end
...
data_RT = [vp_RT.subject_1; vp_RT.subject_2; vp_RT.subject_3;...
           ...
           vp_RT.subject_16; vp_RT.subject_17; vp_RT.subject_18;...
           vp_RT.subject_19; vp_RT.subject_20; vp_RT.subject_21;...
           vp_RT.subject_22; vp_RT.subject_23; vp_RT.subject_24;...
           vp_RT.subject_25; vp_RT.subject_26];
...
for column = 1:8
    ...
    % Save figures for estimation of normality
    histogram(table2array(data_RT(:,column)))
    this_name = strcat("RT_condition_",num2str(column));
    saveas(gcf, this_name, 'png');
...
save 'replication_RANOVA_MyAnalysis_all_data_accuracy_data_to_plot.mat' data_plot_accuracy means_data answer_frequencies missing_data_frequency ranova_accuracy;
save 'replication_RANOVA_RT_data_to_plot.mat' data_plot_RT ranova_RT;