...
%% DATA IMPORT
% Import all data in from file named 'Francas_Daten_geordnet'
data = spreadsheetDatastore('Francas_Daten_geordnet.xlsx');
% Determine column names
data.SelectedVariableNames = {'VP_Nr', 'position', 'day', 'visual_stimulus_text',...
                                'visual_stimulus_color', 'congruency', 'correct_answer', ...
                                'answer', 'accuracy', 'vibration', 'stimulus_start_time',... 
                                'voice_onset', 'reaction_time'};
% Determine column types
data.VariableTypes = {'double','double', 'double', 'char', 'char', 'char', 'char', 'char', ...
    ...
    for day = 1:2
        ...
        for modality = 0:1          
            ...
            for counter = 1:2
                ...
                % Find outlier indices using self-written function 'outlier_detection'
                [indices_outlier, number_outlier] = outlier_detection(treatment);
                % Delete outliers from treatment
                treatment(indices_outlier, :) = [];
                % Delete all rows of this condition from data to be replaced by outlier free treatment data
                toDelete = data.accuracy == 1 & ...
                            data.VP_Nr == subjects & ...
                            data.day == day & ...
                            data.vibration == modality & ...
...
outlier_frequency_all = (number_outlier_all/(800*10)*100)

%% Restore Accuracy Dataset with adding the Correct Answered Trials
accuracy_data = [data;data_correct];

% FINAL For plotting accuracy
writetable(accuracy_data, 'replication_FAnalysis_Fdata_to_plot_accuracy.csv');
...
writetable(data, 'replication_FAnalysis_Fdata_to_plot_RT.csv');
data_plot_RT = data;
...
for pointer = 1:10  
    ...
    accuracy_data = [accuracy_data;[vp.accuracy_mean']];
    
end

% Convert to a table
accuracy_data = array2table(accuracy_data, ...
                'VariableNames', {'day1_congruent_tactile0','day1_congruent_tactile1', ...
...
within = table({'1';'1';'1';'1';'2';'2';'2';'2'},...
                {'congruent';'congruent';'incongruent';'incongruent';'congruent';'congruent';...
                'incongruent';'incongruent'},{'T';'V';'T';'V';'T';'V';'T';'V'},'VariableNames',...
                factorNames);

% Fit the repeated measures model
rm = fitrm(accuracy_data,...
    ...
    for day = 1:2
        ...
        for pointer2 = 1:2
            ...
            for modality = 0:1
                ...
                % For conditions with less than 154 values (missing data, outlier etc) 
                % (maximum number of trials in one treatment is apparently 154, even 
                % though in perfect conditions it should have been 100), fill trials up 
                % with NaNs (to be able to form a table with values of the same VP in a row
                while height(condition) < 154
                    condition = [condition; array2table(NaN(1,13),'VariableNames',condition.Properties.VariableNames)];                   
                end
                % Store data of one treatment of one subject, repeat for all others
...
data_RT.Properties.VariableNames = {'day1_congruent_tactile0','day1_congruent_tactile1', ...
                                    'day1_incongruent_tactile0','day1_incongruent_tactile1',...
                                    'day2_congruent_tactile0','day2_congruent_tactile1',...
                                    'day2_incongruent_tactile0','day2_incongruent_tactile1'};
...
% and 'vibration' and their levels
factorNames = {'day','congruency','vibration'};
within = table({'1';'1';'1';'1';'2';'2';'2';'2'},{'congruent';'congruent';'incongruent';'incongruent';'congruent';'congruent';'incongruent';'incongruent'},{'T';'V';'T';'V';'T';'V';'T';'V'},'VariableNames',factorNames);
...
save 'replication_FAnalysis_Fdata_RT_data_to_plot.mat' data_plot_RT ranova_RT;

%% Leicht anders aber Significanz für den Stroopeffect troztdem gegeben.